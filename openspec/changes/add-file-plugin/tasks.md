...
- [ ] 7.3 Run `cargo fmt --check`, `cargo check`, `cargo clippy --all-targets --all-features -- -D warnings`, and `cargo test`, separating environment-only office/GUI failures from source regressions.
  - Reconciled by `bundle-zero-file-offline-engines`: File/plugin/engine checks pass, and all Clippy findings introduced by that change were fixed. This remains open because repo-wide `-D warnings` still reports pre-existing findings in migration, Caffeine, wallpaper, native resources, Quick Launcher, and Screenshot modules.
...
- [ ] 7.6 Manually smoke-test macOS with the actual tray/main UI, drag-and-drop, native picker, LibreOffice and available Word/PDF providers, Automation denial, background queue, cancellation, output collisions, open, Finder reveal, CJK paths, offline conversion, and app restart cleanup.
  - Reconciled by `bundle-zero-file-offline-engines`: real hidden-WebView source smokes prove both built-in directions, CJK/large inputs, background engine lifetime, truthful quality profiles, and native PDF capture. This remains open because the complete signed packaged tray/picker/open/reveal/repair matrix has not run.
- [ ] 7.7 Manually smoke-test Windows with the actual main/tray UI, Explorer paths, LibreOffice and available Word/PDF providers, COM/activation denial, process-tree cancellation, output collisions, open, Explorer reveal, CJK paths, offline conversion, and app restart cleanup.
  - Reconciled by `bundle-zero-file-offline-engines`: Windows CI now builds and audits the platform-independent PDF-to-DOCX engine path and tests that the macOS DOCX-to-PDF provider cannot be selected. This remains open pending a real Windows runtime/device smoke; Windows DOCX-to-PDF is intentionally not claimed by the built-in engine change.
